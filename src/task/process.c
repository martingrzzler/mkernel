...
{
	// usually you save state of video memory or something but in our case all processes are printing to the same video memory
...
void *process_malloc(struct process *process, size_t size)
{
	...
	int res = paging_map_to(process->task->page_directory, ptr, ptr, paging_align_address(ptr + size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
	if (res < 0)
	{
		goto out_err;
	}
...
int process_map_memory(struct process *process)
{
	...
	case PROCESS_FILETYPE_BINARY:
		...
		break;
	default:
		panic("ERROR: process_map_memory: Invalid filetype\n");
	}
	if (res < 0)
	{
		goto out;
