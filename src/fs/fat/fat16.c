...
// converts filename = 'test    ' and extensiopn = 'txt' to 'test.txt
...
{
	// this returns a 32 bit value and high and low overwrite each other
	return (item->high_16_bits_first_cluster) | item->low_16_bits_first_cluster;
}

static int fat16_cluster_to_sector(struct fat_private *private, int cluster)
{
	// 2 is subtracted because it's specified in th fat 16 manual
	// All Data on a Partition starts with Cluster #2 (Right after Root Directory)
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int total, int offset, void *out)
{
	int res = 0;
	struct fat_private *private = disk->fs_private;
	int size_of_clusters_bytes = private->header.primary_header.sectors_per_cluster * disk->sector_size;
	...
	int offset_from_cluster = offset % size_of_clusters_bytes;
	int starting_sector = fat16_cluster_to_sector(private, cluster_to_use);
	int starting_pos = (starting_sector * disk->sector_size) + offset_from_cluster;
	int total_to_read = total > size_of_clusters_bytes ? size_of_clusters_bytes : total;
...
static int fat16_read_internal(struct disk *disk, int starting_cluster, int offset, int total, void *out)
{
	...
	return fat16_read_internal_from_stream(disk, stream, starting_cluster, total, offset, out);
}
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
	...
	for (int i = 0; i < directory->total; i++)
	{
		...
		{
			// Found file let's create new fat_item
