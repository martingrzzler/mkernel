...
{
  print("Program crashed! There was an exception.\n");
...
void idt_clock()
{
  // achknowledge interrupt processed before task switch
  outb(0x20, 0x20);
...
void isr80h_register_command(int command_id, ISR80H_COMMAND command)
{
  ...
  {
    panic("ERROR the command is out of bounds\n");
  }
  if (isr80h_commands[command_id])
  {
    panic("ERROR attempt to overwrite an existing command detected\n");
  }
